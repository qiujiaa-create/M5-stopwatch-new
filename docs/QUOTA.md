...
设备面板分别接受 604800 秒的周额度和可选的 18000 秒 5H 额度。周额度必须有效；5H 缺失、过期或数值异常时显示 `--`，不会用周额度冒充 5H，也不会显示为 0%。Codex Micro 使用内外双环；OpenWatcher V2 在周额度大数字下方显示 5H 剩余和重置倒计时，并在文字上方用横向渐变进度条表达相同的 5H 剩余百分比。未知或过期时进度条只显示暗色轨道。5H 只保存在 Bridge 内存中，不写入设备的持久额度缓存。录音期间获取的新面板会暂存，并在回到空闲后补发。Bridge v1.4.0 的日界线固定为北京时间 08:00 到次日 07:59，不再跟随电脑时区。未配置云同步时，Bridge 在本机维护以下统计：
