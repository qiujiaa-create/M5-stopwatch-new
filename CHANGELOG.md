...
文档版本：1.6；最后更新：2026-09-20 09:53。
...
### V2 TODAY 引导线 — 2026-09-20

- 将 TODAY 区域右侧引导折线由向下改为向上，终点指向周额度剩余数字的下沿，明确 Today 是全周额度的当日消耗比例，不是 5H 额度。
- 本次仅调整 V2 静态画布几何，不改变额度数据、5H 进度条、触摸区域或其它 BLE/HID 行为。ESP-IDF v5.5.4 增量构建、版本检查和差异检查通过；factory 镜像 4,116,656 字节，SHA-256 `77c94539582386cc24096e89bafc2f6c80c7c8034a15a6eb2467895351d8b727`，已通过 `/dev/cu.usbmodem21201` 的 `app-flash` 仅写入 `factory @ 0x20000`，写后 hash 与独立 `verify_flash` 摘要匹配并完成硬复位。实机显示仍待确认。

### V2 5H 剩余额度进度条 — 2026-09-20

- 将 OpenWatcher V2 的 5H 文字上方分隔装饰改为剩余额度进度条；填充长度对应 5H 剩余百分比，颜色由低额红色平滑过渡到高额绿色。
- 5H 数据缺失、过期或异常时只保留暗色轨道，并继续显示 `5H --` / `RESET --`；不使用周额度代替。
- ESP-IDF v5.5.4 全量构建与版本一致性检查通过。factory 应用镜像为 4,116,656 字节，SHA-256 `770c92c102acdbbd631802fe1faedeb9a94ddaf5574f00d7f3c57b5c0b43fa41`；已通过 `/dev/cu.usbmodem21201` 的 `app-flash` 仅写入 factory 应用偏移 `0x20000`，写后 hash 与独立 `verify_flash` 摘要匹配并完成硬复位。实机显示与动态刷新仍待确认。

### V2 中心 Radial 长按 — 2026-09-18

- 中心四向 Radial 进入阈值由 480ms 调整为 280ms，方向映射、连续事件和松手归中逻辑保持不变。
- 主机差异检查与 ESP-IDF v5.5.4 构建通过。本次镜像 4,116,352 字节，SHA-256 `79e6c41ee47159b2ac76d8b283874bc0a8b859f7e002853d327b43a8f078a7ec`；已通过 `/dev/cu.usbmodem21201` 写入 factory 应用槽并通过写后 hash 校验、自动硬复位，中心长按行为仍待实机确认。

### Focus 倒计时设置 — 2026-09-17

- 点击倒计时分钟数字，可在 1–60 分钟间拖动滑块或逐分钟加减；首次默认 25 分钟，保存后由 NVS 记住上次时长。保存新时长将当前倒计时重置到待开始，60 分钟显示为 `60:00`。
- 倒计时下方移除 B 的触屏开始、暂停、继续和复位按钮，改为带小喇叭图标、点按切换的 `RING ON/OFF`；B 实体键控制保持。铃声默认关闭，开启后归零播放一次短提示音，原有振动仍执行。
- 主机计时测试、ESP-IDF v5.5.4 构建及版本检查通过。镜像 4,115,984 字节，SHA-256 `4704941a2035039bd76ab9e00de7308ff80c045e2db021714d64428e559fa052` 已于 2026-09-18 写入 factory 应用槽；刷写命令成功完成并自动硬复位一次。屏幕、按键、铃声与重启后保存仍待实机验收。

### Focus 倒计时铃声 — 2026-09-18

- 倒计时完成提示从三段短音改为六个半秒音符，持续约 3 秒；提示音幅度从 0.7 调整为音频接口支持的最大值 1.0，不改变用户的全局扬声器音量设置。
- 主机计时测试和 ESP-IDF v5.5.4 构建通过。镜像 4,116,000 字节，SHA-256 `8f410119fa2354ecde53e2286a55bdb524bf010b13d0dc29026f6da07062b66e` 已于 2026-09-18 写入 factory 应用槽；刷写命令成功完成并自动硬复位一次。铃声音量与持续时间仍待实机听感确认。

### Focus 倒计时铃声延长 — 2026-09-18

- 保留音符序列、最大提示音幅度、振动和铃声开关，仅将每个音符调整为约 0.667 秒，使完成提示持续约 4 秒。
- 主机计时测试和 ESP-IDF v5.5.4 构建通过。镜像 4,116,000 字节，SHA-256 `23015afe67368363d36f7a9a84d3d6ff0e635a60d1ec44cd9e5a354bf9071fee` 已写入 factory 应用槽；刷写命令成功完成并自动硬复位一次。铃声持续时间仍待实机听感确认。

### Focus 倒计时弧形颜色 — 2026-09-18

- B 倒计时主页面和设置页右上整块弧形半圆改为不透明填充色 `#46D1E0`；正计时珊瑚色和其它计时逻辑保持不变。
- 主机计时测试、差异检查和 ESP-IDF v5.5.4 构建通过。新镜像 4,116,128 字节，SHA-256 `921e30aff211d4a9b4f94115a86deb6cf2eb83d689224aa9228ba3ec400fc1ce`；尚待写入设备与实机确认。

### Focus 按键两步切换 — 2026-09-18

- 在 A 页面按 B 或在 B 页面按 A 时，第一次只切换页面；进入目标页面后再按对应键才启动、暂停或继续计时。
- 主机计时测试、差异检查和 ESP-IDF v5.5.4 构建通过。镜像 4,116,128 字节，SHA-256 `7fa9d34dbc9d403b0b50e912a50485114a082d8b2d078b7f561156f5d93e243a` 已写入 factory 应用槽；刷写命令成功完成并自动硬复位一次。两步切换行为仍待实机确认。

### V2 Agent 长按与执行状态 — 2026-09-18

- 四个 Agent 点的长按提交阈值由 480ms 调整为 280ms。
- Working 状态的小圆点持续脉冲闪动；任务执行期间刷新活动计时，暂时阻止自动变暗、息屏和自动关机，任务结束后恢复原有策略。
- 主机计时测试、差异检查和 ESP-IDF v5.5.4 构建通过。镜像 4,116,352 字节，SHA-256 `8cc1645d1c8538c404ddfd4f7ef7734fec13978c2fee7f3168b62c8591a9deaa`；尚待写入设备与实机确认。

### Focus 本地专注计时 — 2026-09-17

- 在 Launcher 增加独立 Focus 应用：A 正计时、B 固定 25 分钟倒计时，计时只在手表本地进行，两个计时器互斥运行。
- 正计时秒数右侧增加较小的十分之一秒显示；A/B 短按开始、暂停或继续，仅在暂停时长按对应键复位到 `00:00.0` / `25:00`；取消双击结束。触屏 `RESET` 仅在暂停时显示。
- 离开 Focus 或屏幕休眠后继续计时；倒计时归零时显示 `DONE` 并振动一次。设备重启后本地计时状态清空，Codex 页面的 A/B 语音逻辑保持独立。
- 主机计时测试与 ESP-IDF v5.5.4 构建通过；4,111,648 字节 factory 应用镜像 SHA-256 为 `25d5a7e919437c12d3b2c3302f9066277df7858fa48e08b64e802ca89e9c1df5`，`app-flash` 写后哈希校验通过。最新版显示和按键仍待用户完整实机确认。
