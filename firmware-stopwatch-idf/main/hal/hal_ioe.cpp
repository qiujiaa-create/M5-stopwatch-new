...
void Hal::ioe_init()
{
    ...
    // A software switch from Xiaozhi restarts the ESP32 without necessarily
    // removing panel power. Force a full panel power/reset cycle at boot.
    _ioe->digitalWrite(PY32_OLED_RST_PIN, 0);
    _ioe->digitalWrite(PY32_L3B_EN_PIN, 0);
    delay(100);
    ...
    // Check the physical pin, not just the output register write.
    ...
    if (!l3b_ready) {
        mclog::tagError(_tag, "display power enable did not go high");
    }
    delay(150);
